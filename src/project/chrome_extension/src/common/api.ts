...
export const POST = async (host: string, path = '', data?: { [key: string]: any } | string) => {
  ...
  if (path !== '') url.pathname = path;
  try {
    console.debug("[POST]", url.toString(), typeof (data))
...
export const POST_Form = async (host: string, path = '', data?: { [key: string]: any }) => {
  const url = new URL(host)
  console.debug("POST_Form", url.toString(), path, data, host)
  if (path !== '') url.pathname = path;
  try {
    console.debug("[POST_Form]", url.toString(), data)
...
export const DELETE_Form = async (host: string, path = '', data?: { [key: string]: any }) => {
  const url = new URL(host)
  console.debug("DELETE_Form", url.toString(), path, data, host)
  if (path !== '') url.pathname = path;
  try {
    const form = new FormData()
    if (data !== undefined) {
    ...
    throw new Error(`delete Failed: ${url.toString()} / ${err}`)
  }
}